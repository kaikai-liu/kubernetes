...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.StringVar(&c.CPUManagerPolicy, "cpu-manager-policy", c.CPUManagerPolicy, "<Warning: Alpha feature> CPU Manager policy to use. Possible values: 'none', 'static', 'plugin', 'plugin:<policy>'. Default: 'none'")
	fs.Var(flag.NewMapStringString(&c.CPUManagerPolicyConfig), "cpu-manager-policy-config", "Opaque CPU policy (plugin) configuration parameters. Will be collected and passed as a map[string]string to the CPU Manager policy (plugin). There is no default setting for this option.")
